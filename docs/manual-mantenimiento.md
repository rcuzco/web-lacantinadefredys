...
1. Ve a [https://code.visualstudio.com](https://code.visualstudio.com) y descarga el instalador.
2. Instálalo como cualquier programa de Windows.
...
3. Navega hasta la carpeta del proyecto (donde está `index.html`) y haz clic en **Seleccionar carpeta**.
...
Pulsa **Ctrl + S** para guardar. El cambio se aplicará automáticamente en toda la página (teléfono, pie de página, botón de contacto, etc.) sin tocar ningún otro archivo.
...
Para encontrar un plato concreto, usa la búsqueda de VS Code: **Ctrl + F** y escribe el nombre del plato.
...
Para buscar un plato usa **Ctrl + F** con el nombre del plato.
...
Selecciona el bloque completo desde `<div class="menu-item">` hasta su `</div>` de cierre y bórralo. Usa **Ctrl + F** para localizar el plato por su nombre.
...
Busca (**Ctrl + F**) el texto `dia-price-num`. Encontrarás esto:
...
1. Copia el nuevo archivo de imagen en `assets/raw/photos_webp/`.
2. Localiza la foto que quieres sustituir buscando (**Ctrl + F**) el nombre del archivo actual.
...
Busca (**Ctrl + F**) el texto `Instagram de La Cantina`. Encontrarás este bloque:
...
> **Importante:** Las redes sociales aparecen **dos veces** en la página: una en la sección de Contacto y otra en el pie de página. Busca con **Ctrl + F** el texto `Instagram` y repite el cambio en ambas apariciones.
...
En `index.html` busca (**Ctrl + F**) el texto `map-frame`. Encontrarás:
...
Una vez guardados los cambios (**Ctrl + S**):

1. Abre el archivo `index.html` con doble clic (se abrirá en el navegador).
2. Si ya lo tenías abierto, pulsa **F5** o **Ctrl + R** para recargar la página.
...
|---|---|---|
| La página se ve en blanco o rota | Se borró o modificó código HTML por error | Cierra sin guardar (Ctrl + Z para deshacer) y vuelve a intentarlo |
...
| Los cambios no se ven en el navegador | El archivo no se guardó | Pulsa Ctrl + S antes de recargar el navegador |
